...
void Bitmap::loadImage(fstream &file)
{
    ...
    bitmapArray = new dvec3[bitmapSize];
    ...
    originalBitmapArray = new dvec3[bitmapSize];
...
dvec2 Bitmap::getPixelPositionOnScreen(const int l, const int c) const
{
    ...
    dvec2 pos = {newC + width / 2, newL + height / 2};
    return pos;
}
...
void Bitmap::_scaleImage(const float scale)
{
    ...
    for (int l = 0; l < height; l++)
    {
        for (int c = 0; c < width; c++)
        {
            ...
            dvec3 color = originalBitmapArray[oldIdx];
...
void Bitmap::resetImage()
{
    ...
    bitmapArray = new dvec3[bitmapHeader.BiHeight * bitmapHeader.BiWidth];
    memcpy(bitmapArray, originalBitmapArray, sizeof(dvec3) * bitmapHeader.BiHeight * bitmapHeader.BiWidth);
